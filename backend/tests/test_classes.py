...
def test_create_class_teacher(client, db, teacher_headers):
    body = {
        "name": "뮤지컬 B반",
        "description": "기초반",
        "schedule": "화/목 14:00",
...
def test_update_class(client, seed_class, teacher_headers):
...
def test_delete_class(client, seed_class, teacher_headers):
    res = client.delete("/api/classes/c1", headers=teacher_headers)
    assert res.status_code == 200
    assert res.json()["message"] == "Class deleted"

    # Confirm gone
    res2 = client.get("/api/classes/c1", headers=teacher_headers)
...
def test_add_student(client, seed_class, teacher_headers):
    # Remove s2 first to re-add
    client.delete("/api/classes/c1/students/s2", headers=teacher_headers)
...
def test_remove_student(client, seed_class, teacher_headers):
    res = client.delete("/api/classes/c1/students/s2", headers=teacher_headers)
    assert res.status_code == 200
