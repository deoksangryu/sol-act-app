    ...
    try {
      if (editingClass) {
        const updated = await classApi.update(editingClass.id, {
          name, description, schedule, subjectTeachers, studentIds: selectedStudentIds,
        });
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto">
             ...
             <div className="space-y-4">
                ...
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="w-full bg-brand-500 text-white py-3 rounded-xl font-bold hover:bg-brand-600 transition-colors shadow-lg shadow-brand-200 mt-4 disabled:opacity-50"
                >
                  {saving ? '저장 중...' : '저장하기'}
                </button>
             </div>
